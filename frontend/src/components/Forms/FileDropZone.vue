    ...
    new Dropzone('#demo-upload',
      {
        ...
        addRemoveLinks: true,            // Enable file removal link
        autoProcessQueue: false,         // Do not auto-upload files
        init: function () {
          // Reference to the Dropzone instance
          var dz = this;

          // Event listener for when a file is added
          this.on("addedfile", function (file) {
            // Remove previous file if a new file is added
            if (dz.files[1] != null) {
          ...
          this.on("removedfile", function (file) {
            // Logic to handle file removal can go here
            console.log("File removed:", file);
          });

          // Event listener for file upload success
          ...
          this.on("error", function (file, errorMessage) {
            // Logic to handle upload error
            console.log("Error uploading file:", errorMessage);
          });

          // Event listener for the submit button (or any custom trigger)
...
<template>
  <div class="p-6.5">
    <form
      class="dropzone cursor-pointer rounded-md !border-dashed !border-bodydark1 bg-gray hover:!border-primary dark:!border-strokedark dark:bg-graydark dark:hover:!border-primary"
