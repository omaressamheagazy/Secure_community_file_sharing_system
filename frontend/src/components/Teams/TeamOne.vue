...
<template>
  <div class="p-4 sm:p-6 xl:p-9">
    <div
      class="grid grid-cols-1 gap-15 py-4 sm:grid-cols-2 sm:py-6 xl:grid-cols-4 xl:py-7.5 2xl:py-15"
    >
      <template v-for="(item, index) in teamItems" :key="index">
        <!-- team item -->
        <div>
          <div class="relative z-1 mx-auto h-[170px] w-full max-w-[170px] rounded-full">
            <img class="rounded-full" :src="item.image" alt="team" />
          ...
          <div class="my-5.5 text-center">
            <h4 class="mb-0.5 text-lg font-bold text-black dark:text-white">{{ item.name }}</h4>
            <p class="text-xs font-medium">{{ item.position }}</p>
          </div>

          <div class="flex items-center justify-center gap-5.5">
            <router-link to="#" class="text-bodydark2 hover:text-primary">
              <svg
                class="fill-current"
                width="20"
                height="20"
