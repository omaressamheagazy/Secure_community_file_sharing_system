...
<template>
  <div class="p-4 sm:p-6 xl:p-9">
    <div class="grid grid-cols-1 gap-x-7.5 gap-y-7.5 sm:gap-y-12.5 xl:grid-cols-2 xl:gap-y-17.5">
      <template v-for="(item, index) in faqItems" :key="index">
        <!-- faq item -->
        <div class="flex gap-6">
          <div
            class="flex h-15 w-full max-w-15 items-center justify-center rounded-xl bg-primary text-white"
          >
            <svg
              class="fill-current"
              width="33"
          ...
          <div class="flex w-full flex-col gap-6">
            ...
            <p class="w-full max-w-[452px] font-medium">
              {{ item.details }}
            </p>
          </div>
        </div>
