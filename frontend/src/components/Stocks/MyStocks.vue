...
import BrandTwo from '@/assets/images/brand/brand-01.svg'
import BrandThree from '@/assets/images/brand/brand-10.svg'
import BrandFour from '@/assets/images/brand/brand-13.svg'
...
<template>
  <div
    class="col-span-12 rounded-sm border border-stroke bg-white px-5.5 py-7.5 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-5"
  >
    <div class="mb-7.5 flex flex-wrap items-center justify-between gap-2 px-2">
      <div>
        <h4 class="text-title-sm2 font-bold text-black dark:text-white">My Stocks</h4>
      </div>
    ...
    <div class="flex flex-col">
      <template v-for="(item, index) in stocksItems" :key="index">
        <!-- stock item start -->
        <div
          class="flex items-center justify-between rounded-[5px] px-4 py-3.5 hover:bg-[#F8FAFD] dark:hover:bg-meta-4"
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              <h5 class="text-sm font-bold leading-6 text-black dark:text-white">
                {{ item.name }}
              </h5>
              <p class="text-xs font-medium">{{ item.share }} Shares</p>
            </div>
          </div>

          <div class="text-right">
            <p class="mb-1 font-medium text-black dark:text-white">${{ item.price }}</p>
            <p
              class="flex items-center justify-end gap-1 text-xs font-medium"
