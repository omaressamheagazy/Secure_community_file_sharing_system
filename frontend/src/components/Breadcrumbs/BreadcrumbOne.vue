...
// import { ref } from 'vue'
const props = defineProps(['breadcrumbItems', 'path'])
...
<template>
  <div class="py-4 sm:py-6 w-10/12		 xl:py-10">
    <div class="rounded-md border bg-white border-stroke p-4 py-3 dark:border-strokedark sm:py-5.5 sm:px-6 xl:px-7.5">
      <nav>
          ...
          <template v-for="(item, index) in breadcrumbItems" :key="index">
            <li :class="{
              ...
              <router-link v-if="index !== breadcrumbItems.length - 1"
                :to="{ name: path, params: { id: item.entity_id } }" :class="index === 0
                  ? 'flex items-center gap-3.5 font-medium text-black hover:text-primary dark:text-white dark:hover:text-primary'
                  : 'flex items-center gap-3 font-medium'
                  ">
                <span v-html="arrowIcon" v-if="item.id && index !== 0"></span>
                <span :class="{ 'hover:text-primary': index !== breadcrumbItems.length - 1 }">{{
                  item.name
                }}</span>
              </router-link>

              <span v-html="arrowIcon" v-if="index === breadcrumbItems.length - 1 && index !== 0"></span>
              <span v-if="index === breadcrumbItems.length - 1"
                :class="{ 'hover:text-primary': index !== breadcrumbItems.length - 1 }">{{ item.name
                }}</span>
            </li>
          </template>
        </ol>
