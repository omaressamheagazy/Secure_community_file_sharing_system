...
import DropdownDefault from './Dropdowns/DropdownDefault.vue'
import { ref } from 'vue'
    ...
    <div class="px-6 pt-6 pb-7.5">
      ...
      <div class="mb-5.5 flex flex-wrap items-center gap-3.5">
        <template v-for="(tab, index) in tabItems" :key="index">
          <button
            ...
            :class="{
              'bg-primary/[0.08] text-primary border-primary': activeTab === index,
              'border-stroke dark:border-strokedark': activeTab !== index
            }"
      ...
      <div>
        ...
        <div class="mt-4.5 flex flex-col gap-6">
          <template v-for="(item, itemIndex) in tabItems[activeTab].contentItem" :key="itemIndex">
            <div class="grid gap-1 px-4.5 xsm:grid-cols-7 sm:grid-cols-5">
              ...
              <div class="xsm:col-span-2 sm:col-span-1">
                <span
                  :class="{
                    'inline-block rounded bg-meta-8/[0.08] py-1 px-2.5 text-sm font-medium text-meta-8':
                      item.status === 'In Queue',
                    'inline-block rounded bg-meta-3/[0.08] py-1 px-2.5 text-sm font-medium text-meta-3':
                      item.status === 'Sent',
                    'inline-block rounded bg-primary/[0.08] py-1 px-2.5 text-sm font-medium text-primary':
